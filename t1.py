...
class NeuralNet:
    ...
    def training(self, xdata, ydata, iteration=500, alpha=0.1):
        ...
        while i < iteration:
            ...
            if self.j > 100 and i > 5 and self.js[i-4] - self.js[i-1] < 0.001:
                self.js = []
...
def mass_sprint_learning_5_input():
    ...
    nn = NeuralNet(dimIn=5, dimOut=1, ls=10*np.ones(15, dtype=int))
    nn.construct_from_file('mass_spring_nn_5_inputs_20_layers_30k_samples.npz')
    nn.training(input_data, output_data, iteration=1000, alpha=0.001)
    nn.save_network_structure(name='mass_spring_nn_5_inputs_20_layers_30k_samples')
...
def mass_spring_learning_6_input():
    ...
    nn = NeuralNet(dimIn=6, dimOut=1, ls=10*np.ones(20,dtype=int))
    nn.construct_from_file('mass_spring_neural_network.npz')
    nn.training(input_data, output_data, iteration=3000, alpha=0.01)
